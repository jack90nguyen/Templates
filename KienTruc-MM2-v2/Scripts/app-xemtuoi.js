...
let dulieu;
let nguhanh;
$.get('/data/xemtuoi.json', function (data) {
  dulieu = data;
});

$.get('/data/nguhanh.json', function (data) {
  nguhanh = data;
});
...
function submitApp() {
  ...
  console.log(sdau, sduoi, scanh, sgiap);
  console.log(xdau, xduoi, xcanh, xgiap);
  console.log('tamtai', tamtai);
...
function getKimLau(tuoi) {
  const kl = tuoi % 9;
  const kimlau = dulieu.kimlau[kl - 1];
