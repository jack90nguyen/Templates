  ...
  for (let id = 1; id <= rows; id++) {
    ...
    if (source !== undefined) {
      if (source.indexOf("category") !== -1) {
        name = `Danh mục số ${id}`;
        desc = `Đây là đoạn nội dung mẫu để kiểm tra giao diện số ${id}`;
      } else if (source.indexOf("service") !== -1) {
        name = `Dịch vụ số ${id}`;
        desc = `Đây là đoạn nội dung mẫu để kiểm tra giao diện số ${id} bạn có thể thay đổi trong phần quản trị của website.`;
      } else if (source.indexOf("about") !== -1) {
        name = `Hạng mục số ${id}`;
        desc = `Đây là đoạn nội dung mẫu để kiểm tra giao diện số ${id}`;
    ...
    item = item
      .replace(/#Blog/g, "#Post")
      .replace(/#Element/g, "#Post")
      .replace(/#Product/g, "#Post");
...
function bindDataElement(element) {
  ...
  const source = element.dataset.source;
  let rows = 1;
  if (element.dataset.rows !== undefined) rows = parseInt(element.dataset.rows);
  else if (type === "images") rows = 6;
  else if (type === "images-childs") rows = 24;
  else if (type === "tags") rows = 12;
  else if (type === "nav") rows = 5;
  else if (type === "category") rows = 5;
  else if (type === "language") rows = 2;
    ...
    for (let id = 1; id <= rows; id++) {
      ...
      if (type !== undefined) {
        if (type === "images" || type === "images-childs") {
          ...
          item = item.replace(/#Language.Id/g, id);
          item = item.replace(/#Language.Name/g, id === 1 ? "Vietnamese" : "English");
        }
      }
      strContent += item;
    }
